...
class Instance(object):
    ...
    def __init__(self, priorityBoost=False):
        ...
        self._objects:dict[int, Object] = WeakValueDictionary()
        self._estruct = NormEventStruct()
        self._sessionIndex2Session:dict[Hashable, Session] = {}
    ...
    def destroySession(self, sessionHandle:int):
        ...
        if session:=self._sessions.get(sessionHandle):
            ...
            if session._index:
                self._sessionIndex2Session.pop(session._index)
            return session
        
            
    def destroySessionByIndex(self, index:typing.Hashable):
        if session:=self._sessionIndex2Session.get(index):
            self._sessionIndex2Session.pop(index)
    ...
    def createSession(self, address:str, port:int, localId:int=None, index:Hashable=None ):
        ...
        if index:
            self._sessionIndex2Session[index] = session
        return session
