...
def parse_log(file):
    ...
    for line in file:
        ...
        if line.startswith('REPORT'):
            rep = Report()
            hour, min, sec, usec, rep.node = list(map(int,
            ...
            rep.local.txrate, rep.local.sent_rate, rep.local.grtt = list(map(float,
                    LOCAL_RATE_RE.match(line).groups()))

        elif line.startswith('clr'):
            match = LOCAL_CC_RE.match(line).groups()
            rep.local.clr = int(match[0])
            rep.local.rate, rep.local.rtt, rep.local.loss = list(map(float,
                    match[1:]))
            ...
            sender.rate, sender.goodput = list(map(float,
                    REMOTE_RATE_RE.match(line).groups()))

        elif line.startswith('rxObjects'):
            sender.completed, sender.pending, sender.failed = list(map(int,
                    REMOTE_OBJ_RE.match(line).groups()))

        elif line.startswith('fecBufferUsage'):
            sender.fec_cur, sender.fec_peak, sender.fec_over = list(map(int,
                    REMOTE_FEC_RE.match(line).groups()))

        elif line.startswith('strBufferUsage'):
            sender.str_cur, sender.str_peak, sender.str_over = list(map(int,
                    REMOTE_STR_RE.match(line).groups()))

        elif line.startswith('resyncs'):
            sender.resyncs, sender.nacks, sender.supressed = list(map(int,
